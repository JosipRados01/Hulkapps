 ...
 */
 ...
 const gameFields = game.childNodes as NodeListOf<HTMLElement>;
 let moves = []
 ...
 * @return
  ...
  *
  */
 const setPlayerNames = (): void => {
    const inputs = document.querySelectorAll(".overlay input") as NodeListOf<HTMLInputElement>;
    console.log(inputs);
    const names: string[] = [];
    inputs.forEach(inp => { names.push(inp.value); });
    console.log(names);
    (document.getElementById("player1Name") as HTMLElement).innerText = `${names[0]} (x): `;
    if (names.length === 2) {
        (document.getElementById("player2Name") as HTMLElement).innerText = `${names[1]} (o): `;
    } else {
        (document.getElementById("player2Name") as HTMLElement).innerText = "Computer";
    }
}
  ...
  * @return
  */
 const playSound = (ig: string): void => {
    if (ig === "x") {
        // Play X sound
        (document.getElementById("xSound") as HTMLAudioElement).play();
    } else {
        // Play O sound
        (document.getElementById("oSound") as HTMLAudioElement).play();
    }
};
 ...
 const checkWin = (): boolean => {
    const xFields = moves.filter(move => move.value === 1).map(move => move.index);
    const oFields = moves.filter(move => move.value === -1).map(move => move.index);
    ...
    const isWinningCombo = (combo: string[]): boolean => combo.every(el => xFields.includes(el)) || combo.every(el => oFields.includes(el));

    if (winningCombinations.some(isWinningCombo)) {
        const winningIndex = winningCombinations.findIndex(isWinningCombo);
